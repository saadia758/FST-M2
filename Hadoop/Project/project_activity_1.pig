-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/bbhusan/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/bbhusan/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/bbhusan/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/bbhusan/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/bbhusan/outputs' USING PigStorage('\t');